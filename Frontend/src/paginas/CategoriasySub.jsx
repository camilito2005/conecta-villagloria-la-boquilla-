...
export function GestionCategorias() {
  ...
  const abrirModalCategoria = (categoria = null) => {
    setCategoriaEditar(categoria ? { ...categoria } : { categoria: "" });
    setMostrarModalCategoria(true);
  };

  const guardarCategoria = async () => {
    if (!categoriaEditar.categoria || categoriaEditar.categoria.trim() === "") {
      setModalData({
        title: "Error",
    ...
    try {
      ...
      if (categoriaEditar.id) {
        // Editar
        resultado = await Putdata(`categorias/${categoriaEditar.id}`, {
          categoria: categoriaEditar.categoria.trim(),
        });
      } else {
        // Crear
        resultado = await Postdata("categorias", {
          categoria: categoriaEditar.categoria.trim(),
        });
      }
  ...
  const eliminarCategoria = async (id) => {
    ...
    const resultado = await Deletedata(`categorias/${id}`);
    if (resultado?.showModal) {
      setModalData(resultado.modal);
      if (resultado.modal.type === "success") {
        cargarCategorias();
  ...
  const guardarSubcategoria = async () => {
    ...
    try {
      ...
      if (subcategoriaEditar.id) {
        // Editar
        resultado = await Putdata(`subcategorias/${subcategoriaEditar.id}`, {
        ...
        // Crear
        resultado = await Postdata("subcategorias", {
          subcategoria: subcategoriaEditar.subcategoria.trim(),
          categoria_id: subcategoriaEditar.categoria_id,
          parent_id: subcategoriaEditar.parent_id || null,
        });
  ...
  const eliminarSubcategoria = async (id) => {
    ...
    const resultado = await Deletedata(`subcategorias/${id}`);
    if (resultado?.showModal) {
      setModalData(resultado.modal);
      if (resultado.modal.type === "success") {
        cargarSubcategorias();
  ...
  const getNombreCategoria = (id) => {
    ...
    return cat ? cat.categoria : "Sin categoría";
  };
  ...
  const getNombreSubcategoriaParent = (id) => {
    ...
    return sub ? sub.subcategoria : "Ninguna";
  };
            ...
            <div className="form-group">
              ...
              >
                ...
                {subcategorias
                  .filter(
                    (s) =>
                      s.categoria_id === subcategoriaEditar.categoria_id &&
                      s.id !== subcategoriaEditar.id
                  )
                  .map((sub) => (
